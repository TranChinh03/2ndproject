...
export class SLRdetect {
  ...
  enableCam(event) {
    ...
    this.outputLabel = document.getElementById("output_Label");
  ...
  async predictWebcam() {
    ...
    if (this.results.gestures.length > 0) {
      ...
      this.outputLabel.innerText = `Your hand gesture with Confidence: ${categoryScore} % `;
      this.gestureOutput.style.display = "block";
      this.gestureOutput.innerText = `${categoryName}\n`;
